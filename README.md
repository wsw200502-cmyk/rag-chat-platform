...
基于 **FastAPI + Ollama + LangChain + React** 构建的本地 RAG 智能对话系统，支持 **9B 三角形多Agent协作审查**、**ReAct Agent**、**HyDE 检索增强**、**Self‑RAG**、**混合检索 + Reranker 重排序**、**SSE 流式对话**、**RAGAS 自动评估**，前端采用初音未来主题。
...
三角形多Agent审查	9B 单模型通过 Prompt 切换“解耦→生成→审查”三阶段，输出审查意见，支持故障接管	/chat/review
...
🧩 项目亮点
多模型协作与 Agent：从轻量对话到深度推理，再到三角形多角色审查（分析师→执行者→审查者）、ReAct Agent 和 Self‑RAG，完整覆盖 Agent 演进路线。
...
生产级可靠性：502 自动重试、嵌入模型预热、工具调用防重复、最终答案兜底策略等，保障系统稳定。三角形架构支持单点故障容错（如 Agent-B 崩溃时 Agent-C 自动接管生成）。
...
/chat/review	三角形多Agent审查（9B 单模型 Prompt 切换，含故障接管）
...
│   └── test_triangle_agent.py # 三角形协作集成测试
├── frontend/
│   ├── index.html
│   ├── package.json
│   ├── vite.config.js
