...
# ==================== 混合检索 + Rerank ====================
def retrieve_with_hybrid_and_rerank(query: str, k_vector=10, k_bm25=10, final_k=3, max_retries=5):
    vec_docs = []
...
# ==================== 构建消息列表 ====================
def build_messages(question: str, history: List[Dict[str, str]], extra_context: str = "") -> list:
...
async def chat_review(req: ChatRequest):
    ...
    decomp_prompt = (
        "你是需求分析专家。将用户需求分解为清晰步骤并生成优化提示词。只输出优化提示词。\n"
        f"用户需求：{req.message}"
    )
    try:
        ...
        return ChatResponse(response=f"需求解耦失败: {str(e)}")
    ...
    try:
        ...
        return ChatResponse(response=f"生成回答失败: {str(e)}")
...
async def chat_hyde(req: ChatRequest):
    try:
        ...
        logger.error(f"HyDE 请求失败: {str(e)}")
        return ChatResponse(response=f"HyDE 处理失败: {str(e)}")
...
def self_rag_answer(question: str, history: List[Dict[str, str]], decision_model: str = "qwen2.5:7b", generation_model: str = "qwen2.5:14b", max_iterations: int = 3) -> str:
    decision_llm = get_llm(decision_model)
    generation_llm = get_llm(generation_model)

    messages = [
        SystemMessage(content=SELF_RAG_SYSTEM_PROMPT)
    ]
    for h in history[-10:]:
        role = h.get("role")
        content = h.get("content", "")
...
async def chat_selfrag(req: ChatRequest):
    try:
        ...
        logger.error(f"Self-RAG 请求失败: {str(e)}")
        return ChatResponse(response=f"Self-RAG 处理失败: {str(e)}")
...
class RobustAgentExecutor:
    ...
    def _parse_action(self, content: str) -> Optional[Dict[str, str]]:
        ...
        final_match = re.search(r'(?:final|最终答案|答案)[:\s]*["\']?(.*?)["\']?\s*$', content, re.IGNORECASE | re.DOTALL)
        if final_match:
    ...
    async def _execute_single_task(self, task, working, episodic, history):
        ...
        while step < self.max_steps and (time.time() - start_time) < self.timeout:
            ...
            if action and "tool" in action:
                ...
                messages.append(HumanMessage(content="请立即输出 final 答案，格式：{\"final\": \"...\"} 或 Final Answer: ..."))
...
async def agent_endpoint(req: ChatRequest):
    try:
        ...
        return ChatResponse(response=f"Agent 运行失败: {str(e)}")
